...
const PlyrPlayer = ({ url }) => {
  useEffect(() => {
    (function plyrPluginCapture(document) {
      ...
      function capture(player, label) {
        ...
        img.onload = function onload() {
          ...
          const screenShotImg = img.src.replace(
            /^data:image\/[^;]+/,
      ...
      document.addEventListener("ready", (event) => {
        ...
        if (
          ...
          config.controls.includes("capture")
        ) {
          ...
          const btn = `
            <button class="plyr__control plyr__control--forward" type="button" data-plyr="capture">
              Screenshot
    ...
    const checkReadyState = setInterval(() => {
      if (document.readyState === "complete") {
        ...
        const plyrFullscreen = document.querySelector(
          '[data-plyr="fullscreen"]',
        );

        const plyrCapture = document.querySelector('[data-plyr="capture"]');
        const plyrMenuContainer = document.querySelector(
          '.plyr__menu .plyr__menu__container [role="menu"]',
        );
  ...
  return (
    <div>
      <video id="player" crossOrigin="true" poster="/images/studio/01.jpg" />
    </div>
  );
};
